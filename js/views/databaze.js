...
function dbCenik(el, kolekce){
  ...
  el.innerHTML = `
    <div class="btn-rada">
      <button class="btn btn-plny" id="dbImport">⬆ Import CSV/XLSX</button>
      <button class="btn" id="dbPridat">+ Ručně</button>
  ...
  function vypis(){
    ...
    seznam.innerHTML = vysledky.length ? vysledky.slice(0, 150).map(p => `
      <div class="radek" data-id="${p.id}" style="cursor:pointer">
        <div class="radek-info">
          <div class="radek-nazev">${U.esc(p.nazev)}${p.kod ? `<span class="stitek">${U.esc(p.kod)}</span>` : ''}</div>
          <div class="radek-sub">${U.kc(p.cena)}/${U.esc(p.jednotka || 'ks')} bez DPH · DPH ${U.num(p.sazbaDph ?? 21)} %</div>
        </div>
      </div>`).join('') + (vysledky.length > 150 ? `<div class="prazdno">…a dalších ${vysledky.length - 150}. Upřesni hledání.</div>` : '')
      : `<div class="prazdno">${seznamDat.length ? 'Nic nenalezeno' : 'Ceník je prázdný – nahraj CSV/XLSX nebo přidej ručně'}</div>`;
...
function cenikPolozkaModal(kolekce, p){
  ...
  const ov = U.modal(`
    ...
    <div class="pole"><label>Sazba DPH %</label><input id="fDph" type="text" inputmode="numeric" value="${x.sazbaDph ?? 21}"></div>
...
async function importModal(cil){
  ...
  const ov = U.modal(`
    ...
    <div class="pole"><label>Výchozí sazba DPH % (když není ve sloupci)</label>
      <input id="iDph" type="text" inputmode="numeric" value="21"></div>

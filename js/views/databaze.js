      ...
      ${skupiny[kat].map(p => {
        ...
        return `<div class="radek" data-id="${p.id}" style="cursor:pointer">
          <div class="radek-info">
            ...
            ${p.trh || p.poznamka ? `<div class="radek-sub">${p.trh ? 'trh: ' + U.esc(p.trh) + ' Kč' : ''}${p.trh && p.poznamka ? ' · ' : ''}${p.poznamka ? U.esc(p.poznamka) : ''}</div>` : ''}
          </div>
